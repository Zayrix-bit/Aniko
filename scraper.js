...
// Manual slug overrides for problematic anime
const SLUG_OVERRIDES = {
  "207141": "chainsmoker-cat" // Dandadan
};
 ...
 */
async function findAnikotoShow(enTitle, jikanData, anilistId) {
  if (anilistId && SLUG_OVERRIDES[anilistId]) {
    const chosenSlug = SLUG_OVERRIDES[anilistId];
    const pageHtml = await httpGet(`${ANIKOTO}/watch/${chosenSlug}`, { Referer: `${ANIKOTO}/` }).catch(() => "");
    const idM = pageHtml.match(/data-id="(\d+)"/);
    if (idM) return { slug: chosenSlug, showId: idM[1] };
  }
...
/**
 * Resolve anime metadata (English title and Jikan data) using AniZip or Jikan API fallback.
 */
async function resolveAnimeMetadata(id) {
  let enTitle = "";
  let jikanShow = null;

  // Try AniZip first (assuming it's an AniList ID)
  const anizip = await getJSON(`${ANIZIP}?anilist_id=${id}`).catch(() => null);
  if (anizip) {
    enTitle = anizip.titles?.en ?? Object.values(anizip.titles ?? {})[0] ?? "";
    const malId = anizip.mappings?.mal_id;
    jikanShow = malId ? await getJSON(`${JIKAN}/anime/${malId}`).catch(() => null) : null;
  }

  // Fallback: If AniZip failed, maybe it's a MAL ID. Use Jikan directly.
  if (!enTitle) {
    const jikanRes = await getJSON(`${JIKAN}/anime/${id}`).catch(() => null);
    jikanShow = jikanRes;
    enTitle = jikanShow?.data?.title_english ?? jikanShow?.data?.title ?? "";
  }

  if (!enTitle) {
    throw new Error(`Could not resolve English title for ID: ${id} via AniZip or Jikan`);
  }

  return { enTitle, jikanShow };
}

/**
 * Find a specific episode on Anikoto using AniList ID or MAL ID
 */
async function getAnikotoEpisode(id, epNum) {
  let enTitle = "";
  let jikanShow = null;

  // If we have an override, we don't need to resolve metadata
  if (!SLUG_OVERRIDES[id]) {
    const meta = await resolveAnimeMetadata(id);
    enTitle = meta.enTitle;
    jikanShow = meta.jikanShow;
  }

  const { showId, slug } = await findAnikotoShow(enTitle, jikanShow, id);
...
async function extractRawAnikotoStreams(anilistId, audio, epNum) {
  let ep = null;
  try {
    ep = await getAnikotoEpisode(anilistId, epNum);
  } catch (err) {
    console.error(`Anikoto episode extraction failed: ${err.message}`);
  }
...
async function getWatch(anilistId, audio, epNum) {
  ...
  if (mega?.sources?.file) {
    // streams.push({ url: mega.sources.file, type: "hls", referer: `${MEGAPLAY}/`, server: "Megaplay", priority: 5 });
  }
  streams.push({ url: embedUrl, type: "embed", referer: `${MEGAPLAY}/`, server: "Megaplay-embed", priority: 4 });

  if (vidtube?.sources?.file) {
    // streams.push({ url: vidtube.sources.file, type: "hls", referer: `${VIDTUBE}/`, server: "Vidtube", priority: 7, default: true });
  ...
  if (vidwish?.data?.sources?.file) {
    // streams.push({ url: vidwish.data.sources.file, type: "hls", referer: `${VIDWISH}/`, server: "VidWish", priority: 6 });
  }
  if (realId) {
  ...
  let mapper = null;
  const hasFastHls = streams.length > 0;
  if (!hasFastHls) {
    try {
      const { enTitle, jikanShow } = await resolveAnimeMetadata(anilistId);
      const { showId } = await findAnikotoShow(enTitle, jikanShow, anilistId);
...
async function getEpisodes(anilistId, ctx = {}) {
  ...
  // Has MAL ID → use Jikan episodes (all pages)
  ...
  try {
    ...
    anikotoEpList.forEach((e) => anikotoEpMap.set(e.num, { hasSub: e.hasSub, hasDub: e.hasDub }));
  } catch { }
